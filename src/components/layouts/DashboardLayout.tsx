...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const navigation = [
    ...
    { name: 'Earnings', href: '/earnings', icon: EarningsIcon },
    { name: 'Support', href: '/support', icon: SupportIcon },
    { name: 'Profile', href: '/profile', icon: ProfileIcon },
    { name: 'Theme Settings', href: '/settings/theme', icon: ThemeIcon },
  ];
      ...
      <div className={`fixed inset-0 flex z-40 md:hidden ${sidebarOpen ? 'block' : 'hidden'}`} role="dialog" aria-modal="true">
        ...
        <div className="relative flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-white">
          ...
          <div className="flex-shrink-0 flex items-center px-4">
            <Image src="/logo512.png" alt="Dabbzo Logo" width={32} height={32} className="mr-2" />
            ...
            <nav className="px-2 space-y-1">
              {navigation.map((item) => (
                <Link
                  ...
                  style={{
                    backgroundColor: pathname === item.href ? `${colors.primary}20` : '',
                    color: pathname === item.href ? colors.primary : ''
                  }}
                >
                  <item.icon
      ...
      <div className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0">
        <div className="flex-1 flex flex-col min-h-0 border-r border-gray-200 bg-white">
          <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
            ...
            <nav className="mt-5 flex-1 px-2 bg-white space-y-1">{navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
          ...
          <div className="flex-shrink-0 flex border-t border-gray-200 p-4">
            <div className="flex-shrink-0 w-full group block">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium text-gray-700 group-hover:text-gray-900">Vendor Name</p>
...
function EarningsIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}>
